...
def rank_CpGs(config):
    ...
    naipu_ranked_CpGs = pd.read_csv(
        base_path + "out_gene_rankings_aml.csv", sep=" ", header=None
    )
    ...
    ranked_CpGs_df.to_csv(
        base_path + "ranked_CpGs_gnn_niapu.csv", sep="\t", index=False
    )
