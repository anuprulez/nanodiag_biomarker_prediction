...
def draw_xai_graph(G, sorted_ranking, idx_global, config):
    """
    ...
    print(f"s_rankings_draw: {len(s_rankings_draw)}")
    ...
    _save(
        fig,
        plot_local_path
        / f"Explaination_subgraph_node_{idx_global}_edges_{n_edges}_epo_{n_epo}_{config.model_type}.pdf",
        dpi,
    )
    plt.close()
